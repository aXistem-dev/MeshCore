...
void CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {  // Legacy prefs loader
  ...
  if (file) {
    ...
    if (file.available() >= (int)(sizeof(_prefs->flood_max_unscoped) + sizeof(_prefs->flood_max_advert))) {
      file.read((uint8_t *)&_prefs->flood_max_unscoped, sizeof(_prefs->flood_max_unscoped));   // 291
      file.read((uint8_t *)&_prefs->flood_max_advert, sizeof(_prefs->flood_max_advert));       // 292
    } else {
      _prefs->flood_max_unscoped = 0xFF;
      _prefs->flood_max_advert = 8;
    }
    if (file.available() >= (int)(sizeof(_prefs->radio_fem_rxgain) + sizeof(_prefs->cad_enabled))) {
...
void CommonCLI::handleCommand(uint32_t sender_timestamp, char* command, char* reply) {
    ...
    if (memcmp(command, "poweroff", 8) == 0 || memcmp(command, "shutdown", 8) == 0) {
      ...
      if (strcmp(key, "telem_loc_policy") == 0) {
        ...
        if (strcmp(key, "gps_saver_mode") == 0) {
          const char* actual = _sensors->getSettingByKey("gps_saver_mode");
          if (actual) {
            int m = atoi(actual);
            _prefs->gps_saver_mode = (uint8_t)(m >= 0 && m <= 2 ? m : 2);
            savePrefs();
          }
    ...
    } else if (memcmp(command, "gps hold", 8) == 0) {
      if (strlen(command) == 8) {
        ...
        if (v >= 5 && v <= 240) {
          ...
          _sensors->setSettingValue("gps_saver_hold", command + 9);
          savePrefs();
          strcpy(reply, "ok");
        } else {
          strcpy(reply, "error");
