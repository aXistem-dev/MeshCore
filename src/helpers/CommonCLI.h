...
public:
  ...
  uint8_t gps_saver_hold = 15;  // 5–240 s, default 15
  uint8_t gps_timeout_min = 5;  // 1–15 min, default 5
  #endif
  uint8_t telem_loc_policy = TELEM_LOC_ALLOW;  // 0=deny, 1=allow (non-guest), 2=always
...
private:
  ...
  protected:
    void structure() override {
      ...
      #if GPS_POWER_SAVE_ACTIVE
      def("saver", _parent->gps_saver_mode);
      def("saver_hold", _parent->gps_saver_hold);
      def("saver_timeout", _parent->gps_timeout_min);
      #endif
    }
  public:
...
protected:
  void structure() override {
    ...
    def("lat", node_lat);
    def("lon", node_lon);
    def("telem_loc_policy", telem_loc_policy);
    def("radio", radio);
    def("bridge", bridge);
    def("gps", gps);
    def("repeat", repeat);
