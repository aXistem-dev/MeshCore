  ...
  if (_display != NULL) {
    if (!_display->isOn()) {
      if (!hasConnection()) {
        ...
        if (_node_prefs) {
          if (_node_prefs->screen_brightness > 3) {
            _node_prefs->screen_brightness = 2;
          }
          _display->setBrightness(_node_prefs->screen_brightness);
        }
      }
...
void UITask::loop() {
  ...
  if (c != 0 && curr) {
    // If screensaver is active, exit it first and consume the input
    bool was_screensaver = (curr == screensaver);
    if (was_screensaver) {
      gotoHomeScreen();
      _auto_off = millis() + AUTO_OFF_MILLIS;   // extend auto-off timer
      _next_refresh = 100;  // trigger refresh
      // Don't process input further - it was only used to exit screensaver
    } else {
      curr->handleInput(c);
      _auto_off = millis() + AUTO_OFF_MILLIS;   // extend auto-off timer
      _next_refresh = 100;  // trigger refresh
...
char UITask::checkDisplayOn(char c) {
  if (_display != NULL) {
    if (!_display->isOn()) {
      ...
      if (_node_prefs) {
        // Validate and fix corrupted values (defensive programming)
        if (_node_prefs->screen_brightness > 3) {
          _node_prefs->screen_brightness = 2;  // Default to Normal
        }
        _display->setBrightness(_node_prefs->screen_brightness);
      }
      c = 0;
