...
public:
  ...
  uint8_t  screen_always_on = 0;
  uint8_t  screen_brightness = 2;  // 0=Dim, 1=Low, 2=Normal, 3=Bright
  uint8_t  screen_screensaver = 0;  // 0=off, 1=on (only works if screen_always_on is enabled)
  int16_t  timezone_offset_minutes = 0;  // Timezone offset in minutes from UTC (e.g., +60 for UTC+1, -300 for UTC-5)
...
private:
  ...
  protected:
    void structure() override {
      ...
      def("scr_on", _parent->screen_always_on);
      def("scr_br", _parent->screen_brightness);
      def("scr_ss", _parent->screen_screensaver);
      def("tz", _parent->timezone_offset_minutes);
    }
  public:
