...
uint8_t SensorMesh::handleRequest(uint8_t perms, uint32_t sender_timestamp, uint8_t req_type, uint8_t* payload, size_t payload_len) {
  ...
  if (req_type == REQ_TYPE_GET_TELEMETRY_DATA) {  // allow all
    ...
    // Safety check: ensure reply fits in packet payload
    if (4 + tlen > MAX_PACKET_PAYLOAD) {
      MESH_DEBUG_PRINTLN("handleRequest - ERROR: Reply too large! %d bytes (max %d)", 4 + tlen, MAX_PACKET_PAYLOAD);
      return 0;  // Invalid
    }
