...
# Script to update dev-slunsecore branch with latest changes from main
# Run this whenever main gets updated from upstream

set -e

echo "=== Updating dev-slunsecore with latest main ==="
echo ""
...
DEV_SLUNSECORE_BASE=$(git merge-base dev-slunsecore main 2>/dev/null || echo "")

if [ "$MAIN_HEAD" = "$DEV_SLUNSECORE_BASE" ]; then
    echo ""
    echo "✓ dev-slunsecore is already up-to-date with main"
...
git log --oneline ${DEV_SLUNSECORE_BASE}..main

# Update dev-slunsecore
echo ""
echo "4. Updating dev-slunsecore branch..."
git checkout dev-slunsecore

# Try merge first (preserves history)
echo "   Merging main into dev-slunsecore..."
if git merge main --no-edit; then
    echo ""
    echo "✓ Successfully merged main into dev-slunsecore"
    echo ""
    ...
    echo "  - Test the build"
    echo "  - Push to origin: git push origin dev-slunsecore"
else
    echo ""
